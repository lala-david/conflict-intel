...
def save_events(data: dict, date_str: str):
    ...
    for e in data.get("gdelt", []):
        ...
        try:
            conn.execute(
                "INSERT OR IGNORE INTO events (id, source, date, event_type, sub_event_type, actor1, actor2, country_code, location, latitude, longitude, source_url, enrichment, collected_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (event_id, "gdelt", e.get("date", ""), e.get("event_code", ""), "",
                 e.get("actor1", ""), e.get("actor2", ""), e.get("country_code", ""),
                 e.get("location", ""),
...
def save_daily_stats(date_str: str, data: dict, enrichment_stats: dict):
    ...
    try:
        conn.execute(
            "INSERT OR REPLACE INTO daily_stats (date, gdelt_count, acled_count, news_count, expert_count, sanctions_count, sanctions_new, total_fatalities, org_matches, country_matches, zone_matches, top_countries, top_actors) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (date_str,
             len(data.get("gdelt", [])),
             len(data.get("acled", [])),
